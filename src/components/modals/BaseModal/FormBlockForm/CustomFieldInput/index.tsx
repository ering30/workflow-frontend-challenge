...
const CustomFieldInput = (props: CustomFieldInputProps) => {
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement>, fieldType: string) => {
    if (fieldType === 'required') {
      const checked = (e.target as HTMLInputElement).checked;
      setModalData(prevData => ({
        ...prevData,
        fields: prevData.fields?.map(fieldItem => 
          fieldItem.id === props.id 
            ? { ...fieldItem, required: checked }
            : fieldItem
        ) || []
      }));
    } else {
  ...
  return (
    <Flex
      className="flex-col bg-gray-100 border-gray-500 rounded-md p-4 gap-2 relative"
    >
      <Flex justify="end">
        <Button 
      ...
      <Form.Field name="fieldName" className="flex flex-col gap-2" >
        <Form.Label htmlFor="fieldName" className="text-sm text-gray-400">Field Name</Form.Label>
      ...
      <Flex justify="between" align="center" className='gap-2'>
        <Form.Field name="fieldType" className="flex flex-1 flex-col gap-2 max-w-[50%]" >
          <Form.Label htmlFor="fieldType" className="text-sm text-gray-400">Field Type</Form.Label>
          <Form.Control asChild>
            <select 
              name="type" 
              id="fieldType" 
        ...
        <Form.Field name="fieldRequired" className="flex flex-1 gap-2 justify-center max-w-[50%]" >
          <Form.Label htmlFor="fieldRequired" className="text-sm text-gray-400">Required</Form.Label>
          <Form.Control asChild>
            <input 
              type='checkbox' 
              name="fieldRequired" 
